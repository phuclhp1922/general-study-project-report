...
\begin{itemize}
    \item The \textbf{Smoke Map}: Represents the rapid propagation of toxic gases. If a zone is heavily concentrated with smoke, it triggers the dynamic ``smoke factor'' constraint ($\sigma_{a,t}$), immediately altering the zone's accessibility and prioritizing it for evacuation.
...
The algorithm was tested on a multi 2D grid representing a multi-story building. 
Each grid represent an area where sensors work.
...
To satisfy the operational hierarchy of emergency management, the solver was programmed to execute a strict lexicographic optimization. 
...
\begin{itemize}
    ...
    \item \textbf{Phase 2 (Property Protection)}: With $O_1$ locked, the solver processes the Fire Map to route the Suppression Teams, optimizing for the minimum possible property loss and prevention of fully burnt zones ($O_2$). This optimal value is then locked.
